...
function validateFileName(value: unknown) {
  if (typeof value !== 'string' || !value.trim()) throw new Error('A supporting-document file name is required.')
  const originalName = value.trim()
  if (originalName.length > 255) throw new Error('A supporting-document file name is too long.')
  const extension = originalName.split('.').pop()?.toLowerCase() ?? ''
  if (!ALLOWED_EXTENSIONS.has(extension)) throw new Error(`${originalName}: unsupported file type.`)
  return originalName
}
  ...
  fetch: withSupabase(
    ...
    async (req, ctx) => {
      ...
      try {
        ...
        if (!isUuid(quoteId)) throw new Error('Invalid quote ID.')
        ...
        for (let index = 0; index < body.files.length; index += 1) {
          ...
          if (!item || typeof item !== 'object') throw new Error('Invalid supporting-document information.')
          const file = item as Record<string, unknown>
          const originalName = validateFileName(file.originalName)
          const sizeBytes = Number(file.sizeBytes)

          if (!Number.isFinite(sizeBytes) || sizeBytes <= 0) throw new Error(`${originalName}: invalid file size.`)
          if (sizeBytes > MAX_FILE_BYTES) throw new Error(`${originalName}: files must be 10 MB or smaller.`)
          ...
          if (error || !data?.token) {
            console.error('Quote signed upload URL error:', error)
            return Response.json({ ok: false, error: 'Upload authorization could not be created.' }, { status: 500 })
          }
        ...
        console.error('Quote upload authorization validation error:', error)
